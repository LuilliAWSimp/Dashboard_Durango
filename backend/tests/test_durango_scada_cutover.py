...
class DurangoScadaCutoverTests(unittest.TestCase):
    ...
    def test_operational_contract_contains_only_confirmed_sources(self):
        ...
        self.assertEqual([(item['sensor_id'], item['source_key']) for item in JARABES], [(3004, 'TANQUE_FLOW_IN[1]')])
        ...
        self.assertFalse({3002, 3006} & {item.get('sensor_id') for item in ALL_ITEMS})
        self.assertIn(3004, {item.get('sensor_id') for item in ALL_ITEMS})
        self.assertNotIn(3010, {item.get('sensor_id') for item in ALL_ITEMS})
