...
class DurangoHistoryOverviewTests(unittest.TestCase):
    ...
    def test_module_history_uses_confirmed_wells_and_preserves_empty_intervals(self):
        rows=[{'sensor_id':1001,'bucket_start':datetime(2026,8,4,0,0),'samples':15,'flow_avg':10.0,'flow_min':8.0,'flow_max':12.0,'total_open':100.0,'total_close':109.0}]
        with patch.object(history,'_query_15m_multi',return_value=rows),patch.object(history,'_query_physical_validation_rows',return_value=[]),patch.object(history,'query_bos_well_rows',return_value=[]): payload=history.get_water_history_module(module='well',start_date='2026-08-04',end_date='2026-08-04',aggregation='hourly',force_refresh=True)
    ...
    def test_well_module_uses_strict_one_day_bos_fallback(self):
        fallback=[{'operational_ts':datetime(2026,8,4,0,0),'instant_value':4.0,'total_value':200.0},{'operational_ts':datetime(2026,8,4,0,10),'instant_value':5.0,'total_value':204.0}]
        with patch.object(history,'_query_15m_multi',return_value=[]),patch.object(history,'_query_physical_validation_rows',return_value=[]),patch.object(history,'query_bos_well_rows',side_effect=lambda sensor_id,*_:fallback if sensor_id==1051 else []): payload=history.get_water_history_module(module='well',start_date='2026-08-04',end_date='2026-08-04',aggregation='hourly',force_refresh=True)
