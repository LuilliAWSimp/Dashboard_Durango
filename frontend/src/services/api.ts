...
type AuthConfirmation = 'valid' | 'invalid' | 'unknown';
type MeProbeResponse = { browser_session?: string | null; csrf_token?: string | null };
let authConfirmationPromise: Promise<AuthConfirmation> | null = null;

function applyConfirmedSession(data?: MeProbeResponse): void {
...
async function confirmSessionAfterUnauthorized(): Promise<AuthConfirmation> {
  ...
  authConfirmationPromise = (async () => {
    ...
    try {
      const { data } = await authProbe.get<MeProbeResponse>('/auth/me', { headers });
      applyConfirmedSession(data);
      return 'valid' as const;
    } catch (probeError) {
      const status = (probeError as AxiosError)?.response?.status;
      if (status === 401) return 'invalid' as const;
