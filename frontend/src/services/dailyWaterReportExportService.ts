...
export function buildDailyWaterReportHtml(report: DailyWaterReport, logoUrl?: string, documentTitle?: string): string {
  ...
  return `<!doctype html><html lang="es"><head><meta charset="utf-8" /><title>${escapeHtml(fileTitle)}</title><style>
    ...
    <section class="meta"><div class="title"><h1>${escapeHtml(title)}</h1><p>Planta: ${escapeHtml(report.plant || 'Durango')}</p><p>Fecha: ${escapeHtml(report.date || '')}</p></div><div class="report-code"><p>Reporte: <strong>${escapeHtml(report.report_code || '')}</strong></p><p>Estado: Información operativa</p></div></section>
    ...
    ${wellChart}
    ${table('Líneas', [{key:'linea',label:'Línea'},{key:'flujo_lps',label:'Flujo actual',suffix:' L/s'},{key:'volumen_periodo_m3',label:'Volumen periodo',suffix:' m³'},{key:'totalizador_m3',label:'Totalizador',suffix:' m³'},{key:'estado',label:'Estado'}], lines)}
    ${lineChart}
    ${table('Flujos', [{key:'equipo',label:'Punto'},{key:'flujo_lps',label:'Flujo actual',suffix:' L/s'},{key:'volumen_periodo_m3',label:'Volumen periodo',suffix:' m³'},{key:'totalizador_m3',label:'Totalizador',suffix:' m³'},{key:'estado',label:'Estado'}], flows)}
    ${flowChart}
