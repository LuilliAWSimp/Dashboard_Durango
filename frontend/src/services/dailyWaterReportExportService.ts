...
export function buildDailyWaterReportHtml(report: any): string {
  ...
  return `<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${escapeHtml(report.title)}</title><style>
    body{font-family:Arial,sans-serif;color:#152536;margin:30px;background:#fff}h1{text-align:center;color:#073b4c;margin-bottom:6px}h2{color:#075985;margin-top:28px;margin-bottom:10px}.meta{text-align:center;color:#526b78;margin-bottom:24px}.summary{display:grid;grid-template-columns:repeat(5,minmax(150px,1fr));gap:10px}.summary div{border:1px solid #cbdde5;border-radius:10px;padding:12px;background:#f7fbfd}.summary span{display:block;color:#31576a;font-size:11px;text-transform:uppercase;letter-spacing:.06em}.summary strong{display:block;color:#073b4c;font-size:20px;margin-top:6px}.note{margin:12px 0 20px;padding:10px 12px;border-left:4px solid #0b6e8e;background:#eef7fa;color:#31576a;font-size:12px}table{width:100%;border-collapse:collapse;font-size:11px}th{background:#0b6e8e;color:#fff;text-align:center}th,td{border:1px solid #b6cbd6;padding:7px;vertical-align:middle}td:nth-child(n+2){text-align:center}tbody tr:nth-child(even){background:#f3f8fa}@media(max-width:900px){.summary{grid-template-columns:repeat(2,1fr)}table{font-size:10px}}
  </style></head><body>
    <h1>Reporte Diario de Control Hídrico Durango</h1>
    <p class="meta">Periodo: ${escapeHtml(report.period_label)} · Generado: ${escapeHtml(fmtDate(report.generated_at))}</p>
    <div class="summary">
    ...
    ${section('Flujos', report.operational_flows?.rows || [])}
  </body></html>`;
}
