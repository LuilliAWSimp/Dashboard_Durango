...
export default function ModuleHistoryPanel({ range }: Props) {
  ...
  useEffect(() => {
    ...
    if (module === 'tank') { setError(''); return; }
    setSelected(moduleItems[module].map((item) => item.sensorId));
  }, [module]);

  const load = useCallback(async (forceRefresh = false, background = false) => {
    if (module === 'tank' || !range.startDate || !range.endDate) return;
  ...
  useEffect(() => {
    if (module === 'tank' || !includesToday(range)) return undefined;
  ...
  const rows = useMemo<ChartRow[]>(() => {
    ...
    data.series.forEach((series) => series.points.forEach((point) => {
      ...
      row[`flow_${series.sensor_id}`] = point.flow_avg_lps;
      row[`meta_${series.sensor_id}`] = { flowMin: point.flow_min_lps, flowMax: point.flow_max_lps, samples: point.samples, status: point.data_status, discardedEvents: point.discarded_totalizer_events || 0, discardedVolume: point.discarded_volume_m3 || 0, hasDiscontinuities: Boolean(point.has_discontinuities) };
      byTime.set(key, row);
    }));
    return [...byTime.values()].sort((a, b) => a.timestamp - b.timestamp);
  }, [data]);

  const visible = selected.filter((sensorId) => data?.series?.some((series) => series.sensor_id === sensorId));
    ...
    <section className="panel chart-panel fade-up module-history-panel">
      ...
      <div className="module-history-toolbar">
        <div className="module-history-tabs" role="tablist">
          {([['well', 'Pozos'], ['line', 'Líneas'], ['flow', 'Flujos auxiliares'], ['tank', 'Tanques']] as const).map(([value, label]) => (
            <button type="button" role="tab" aria-selected={module === value} className={`module-history-tab ${module === value ? 'active' : ''}`} key={value} onClick={() => setModule(value)}>{label}{value === 'tank' ? <small>Pendiente</small> : null}</button>
          ))}
        </div>
      ...
      {module === 'tank' ? <ChartEmptyState message="Los niveles de tanques están pendientes de validación." /> : <>
        <div className="module-history-sensors">{activeItems.map((item) => <button type="button" className={`sensor-chip ${selected.includes(item.sensorId) ? 'active' : ''}`} key={item.sensorId} onClick={() => toggle(item.sensorId)}>{item.name}</button>)}</div>
        ...
        {rows.length && hasAny && visible.length ? (
          <ResponsiveContainer width="100%" height={390}>
            <LineChart data={rows} margin={{ top: 16, right: 28, bottom: 18, left: 8 }}>
              ...
              {visible.map((sensorId, index) => { const series = data?.series.find((item) => item.sensor_id === sensorId); return <Line key={sensorId} type="linear" dataKey={`flow_${sensorId}`} name={`${series?.name || sensorId} (${series?.flow_unit || 'L/s'})`} stroke={COLORS[index % COLORS.length]} strokeWidth={2.4} dot={false} activeDot={{ r: 4 }} connectNulls={false} isAnimationActive={false} />; })}
            </LineChart>
          </ResponsiveContainer>
