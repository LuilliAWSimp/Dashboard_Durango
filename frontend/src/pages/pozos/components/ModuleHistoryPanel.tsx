        ...
        <ResponsiveContainer width="100%" height={410}>
          <LineChart data={rows} margin={{ top: 16, right: axes.showTotalizer ? 32 : 18, bottom: 18, left: 8 }}>
            ...
            {visible.flatMap((identity) => {
              ...
              if (axes.showTotalizer) chartSeries.push(<Line key={`totalizer-${identity}`} yAxisId="totalizer" type="linear" dataKey={`totalizer_${identity}`} name={`${series?.name || identity} · Totalizador (m³)`} stroke={"#ef4444"} strokeWidth={2.1} strokeDasharray="7 4" dot={false} activeDot={{ r: 4 }} connectNulls={false} isAnimationActive={false} />);
              return chartSeries;
            })}
          </LineChart>
        </ResponsiveContainer>
